...
const { Min } = require('@app/module/combiner');
const { Const } = require('@app/module/generator');

describe('JavaScript/module/combiner/min', () => {
  it("constructed without a sourceModuleA throws an error", () => {
    ...
    // Test
    const testFunc = () => {
      new Min(undefined, sourceModuleB);
    };
  ...
  it("calling getValue without a sourceModuleA throws an error", () => {
    ...
    // Assert
    expect(testFunc).to.throw("Cannot call getValue on min combiner module, sourceModuleA is empty");
  });
  ...
  it("calling getValue without a sourceModuleB throws an error", () => {
    ...
    // Assert
    expect(testFunc).to.throw("Cannot call getValue on min combiner module, sourceModuleB is empty");
  });
});
