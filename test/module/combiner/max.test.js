...
const { Max } = require('@app/module/combiner');
const { Const } = require('@app/module/generator');

describe('JavaScript/module/combiner/max', () => {
  it("constructed without a sourceModuleA throws an error", () => {
    ...
    // Test
    const testFunc = () => {
      new Max(undefined, sourceModuleB);
    };
  ...
  it("calling getValue without a sourceModuleA throws an error", () => {
    ...
    // Assert
    expect(testFunc).to.throw("Cannot call getValue on max combiner module, sourceModuleA is empty");
  });
  ...
  it("calling getValue without a sourceModuleB throws an error", () => {
    ...
    // Assert
    expect(testFunc).to.throw("Cannot call getValue on max combiner module, sourceModuleB is empty");
  });
});
